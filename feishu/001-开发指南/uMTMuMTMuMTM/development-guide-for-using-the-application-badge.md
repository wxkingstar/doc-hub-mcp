<!--
title: 应用角标功能开发指南
id: 7307454685074849796
fullPath: /uAjLw4CM/ukzMukzMukzM/application-badge/development-guide-for-using-the-application-badge
updatedAt: 1740572064000
...
<md-tbody>
<md-tr>
<md-td style="width: 10%">**工作台类型**</md-td>
<md-td style="width: 30%">**如果企业使用的是**
   [定制工作台](https://www.feishu.cn/hc/zh-CN/articles/507240292326)</md-td>
<md-td style="width: 30%">**如果企业使用的是**[网页嵌入工作台](https://www.feishu.cn/hc/zh-CN/articles/854243992870)</md-td>
<md-td style="width: 30%">**如果企业使用的是默认工作台**</md-td>
</md-tr>
<md-tr>
<md-td>**支持的版本**</md-td>
<md-td> **V5.15**</md-td>
...
![image.png](//sf3-cn.feishucdn.com/obj/open-platform-opendoc/04ddc9b47dd53b914ee64ac8f9beeffe_bqimKJTrgl.png?height=512&lazyload=true&width=1280)</md-td>
<md-td>仅主导航栏上工作台右上角展示角标，角标数字是网页内部应用角标去重之和

**注**：网页内部的角标由用户自行实现
![](//sf3-cn.feishucdn.com/obj/open-platform-opendoc/d9febc55ca1a07a32eba54c88d966b80_lZfrrV324w.png?height=631&lazyload=true&width=846)</md-td>
<md-td>**展示在应用图标的右上角：**
- 当应用被添加至 **我的常用**：展示角标
- 当应用被添加至 **分组应用**：展示角标
- 当应用被添加至 **全部应用**：不展示角标
...
![](//sf3-cn.feishucdn.com/obj/open-platform-opendoc/e718413c6884de9a026f30e7cc1d148d_3GhkeVQ51X.png?height=1066&lazyload=true&width=1872)
![](//sf3-cn.feishucdn.com/obj/open-platform-opendoc/4f4e93b54ca5282751b093aceaa61afc_8tdsPFtVJj.png?height=1028&lazyload=true&width=1874)</md-td>
</md-tr>
</md-tbody>
</md-table>
:::
...
<md-table>
  <md-tbody>
   <md-tr>
     <md-td>![](//sf3-cn.feishucdn.com/obj/open-platform-opendoc/f57c129fd39f2a6a54b24ac771943de4_W2JU5xYv07.png?height=850&lazyload=true&width=1888)</md-td>
     <md-td> ![](//sf3-cn.feishucdn.com/obj/open-platform-opendoc/c786ee0bb14ce462a5edc33df83400b5_KtGSgo6N9R.png?height=622&lazyload=true&width=1168)</md-td>
  </md-tr>
</md-tbody>
</md-table>
:::
...
**Step2：用户点击开启应用角标功能后，调用接口[tt.authorize](/ssl:ttdoc/uYjL24iN/ugzMx4COzEjL4MTM)，向用户请求授权**
...
- 应用必须开启小程序或网页功能，才可以使用应用角标功能。因此，需要先按照 [创建企业自建应用](/ssl:ttdoc/home/introduction-to-custom-app-development/self-built-application-development-process) 或 [开发和上架应用商店应用](/ssl:ttdoc/uMzNwEjLzcDMx4yM3ATM/ugzNwEjL4cDMx4CO3ATM) 的流程创建并发布默认功能为小程序或网页功能的应用，流程如下：
	1. 创建应用
	2. 开启应用的小程序或网页功能
...
### 使用应用角标功能
#### 步骤一：应用鉴权
-   小程序：调用小程序接口前需要先进行登录，登录流程详见: [登录小程序](/ssl:ttdoc/uYjL24iN/uETO5QjLxkTO04SM5kDN)
- 网页：调用网页应用接口前，需要先完成鉴权，鉴权流程详见: [H5JSSDK - 鉴权接口](/ssl:ttdoc/uYjL24iN/uEzM4YjLxMDO24SMzgjN)
#### 步骤二：申请用户授权
-   应用角标是需要用户授权后才可以使用的功能，即当用户开启应用角标功能时，应用才可以在工作台展示角标。因此，应用如需使用角标功能，需要先征询用户的授权。
- 应用授权调用 [tt.authorize](/ssl:ttdoc/uYjL24iN/ugzMx4COzEjL4MTM)，用户未设置过权限（首次调用时），会弹窗咨询用户是否授予权限；如果用户设置过权限，会直接返回授权结果，不会展示授权弹窗。
- 调用[tt.authorize](/ssl:ttdoc/uYjL24iN/ugzMx4COzEjL4MTM) 时，传入 应用角标功能对应的 scope 为 `scope.appBadge`。

申请用户授权的方式如下：
#### 判断用户当前的授权状态
在应用内调用 [tt.getSetting](/ssl:ttdoc/uYjL24iN/uQzMx4CNzEjL0MTM) 获取用户已设置（包括授予和拒绝）的权限，如果返回的权限列表中没有`scope.appBadge`，则说明接下来是 首次申请授权；如果包含 `scope.appBadge`，且对应的值为false，则表示用户拒绝授权，同理为true表示用户同意授权。

#### 首次申请权限
在应用内 调用  [tt.authorize](/ssl:ttdoc/uYjL24iN/ugzMx4COzEjL4MTM) 调用接口后，平台会展示授权弹窗征询用户授权（弹窗表现如下图所示），并在用户操作后回调授权状态；
...
#### 用户拒绝后再次开启该权限（可选）
当首次申请授权弹窗，用户没有点击允许时，可以 通过 [tt.openSetting](/ssl:ttdoc/uYjL24iN/uUzMx4SNzEjL1MTM) 接口为用户打开应用设置页面，引导用户在该页面开启应用角标功能，在调用前 可以提前告知用户开启应用角标功能的作用、让用户充分了解功能的价值和必要性；
...
**服务端设置**
-   应用可以通过服务端调用设置应用角标接口来更新角标数字，详见服务端接口文档： [更新应用红点](/ssl:ttdoc/uAjLw4CM/ukTMukTMukTM/application-v6/app_badge/set)
...
- 客户端设置接口为[tt.updateBadge](/ssl:ttdoc/uYjL24iN/uQjMuQjMuQjM/appbadge/updagebadge)，接口调用方式如下：
...
``` 
客户端更新速度快、能够让用户在完成操作后即时看到应用角标的更新。**如果应用同时需要保持不同端的应用角标同步，需要同步调用服务端接口去更新其他端的应用角标数字。**
...
:::
## Q&A
### 调用[tt.updateBadge](/ssl:ttdoc/uYjL24iN/uQjMuQjMuQjM/appbadge/updagebadge) 没能成功更新角标
...
### 网页应用 调用[tt.getSetting](/ssl:ttdoc/uYjL24iN/uQzMx4CNzEjL0MTM) 获取不到 `scope.appBadge` 数据
-   **问题表现**：**网页应用**在window.h5sdk.ready中**立刻**调用 [tt.getSetting](/ssl:ttdoc/uYjL24iN/uQzMx4CNzEjL0MTM)、无法获取到 `scope.appBadge` 的数据，或者立刻调用[tt.authorize](/ssl:ttdoc/uYjL24iN/ugzMx4COzEjL4MTM) 申请 `scope.appBadge` 权限时、会弹出授权弹窗，即使此时用户已经授权过Badge权限。如下图所示：立刻调用时没有获取到 `scope.appBadge` 的数据，设置延时1000ms后调用可以获取到 `scope.appBadge` 的数据：
...
- **问题原因**：JSSDK ready 和 [tt.getSetting](/ssl:ttdoc/uYjL24iN/uQzMx4CNzEjL0MTM) 存在时序问题, getSetting的数据可能还没获取到
- **处理方法**：应用在window.h5sdk.ready后**不要立刻**调用接口，可以设置1000 ms的延时，即在window.h5sdk.ready后间隔1000 ms再调用[tt.getSetting](/ssl:ttdoc/uYjL24iN/uQzMx4CNzEjL0MTM)、[tt.authorize](/ssl:ttdoc/uYjL24iN/ugzMx4COzEjL4MTM)接口
