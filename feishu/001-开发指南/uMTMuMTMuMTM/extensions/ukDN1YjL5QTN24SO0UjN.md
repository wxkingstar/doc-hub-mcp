<!--
title: 聊天框“+”菜单
id: 6907569742383939586
fullPath: /ukTMukTMukTM/ukDN1YjL5QTN24SO0UjN
updatedAt: 1676302882000
source: https://open.feishu.cn/document/client-docs/extensions/“+”-menu-shortcut
-->
# 如何将应用配置到 聊天框“+”菜单
...
1. 首先，开发者需先**开启[小程序](/ssl:ttdoc/home/develop-a-gadget-in-5-minutes/create-a-custom-app)或[网页](/ssl:ttdoc/home/integrating-web-apps-in-5-minutes/create-app-and-configuration)能力**；
...
- 网页应用的AppLink协议可参考[打开网页应用](/ssl:ttdoc/uAjLw4CM/uYjL24iN/applink-protocol/supported-protocol/open-an-h5-app)
- 小程序应用的AppLink协议可参考 [打开小程序](/ssl:ttdoc/uAjLw4CM/uYjL24iN/applink-protocol/supported-protocol/open-a-gadget)
...
#### 小程序应用场景判断
通过[getLaunchOptionsSync](/ssl:ttdoc/uYjL24iN/uAzM1YjLwMTN24CMzUjN) Api 获取LaunchOptions，然后通过[场景值](/ssl:ttdoc/uYjL24iN/uQzMzUjL0MzM14CNzMTN)进行判断
...
在加号菜单场景中，用户点击打开应用时候会随机生成一串数字，称为`triggercode`，它会和当前的会话关联。获取`triggerCode`，可以通过[tt.getHostLaunchQuery](/ssl:ttdoc/uYjL24iN/ugzM4UjL4MDO14COzgTN)；使用`triggerCode`，可以通过 [tt.getTriggerContext](/ssl:ttdoc/no_class/plusmenu/gettriggercontext)  来获取当前会话信息，也可以通过  [tt.sendMessageCard](/ssl:ttdoc/uYjL24iN/uUjN5UjL1YTO14SN2kTN) 向当前会话发送消息卡片。

1. 通过 [tt.getHostLaunchQuery](/ssl:ttdoc/uYjL24iN/ugzM4UjL4MDO14COzgTN) 获取 triggerCode
...
2. 通过 [tt.sendMessageCard](/ssl:ttdoc/uYjL24iN/uUjN5UjL1YTO14SN2kTN) 发送卡片
...
卡片内容json示例，详细配置请参考[消息卡片](/ssl:ttdoc/ukTMukTMukTM/uczM3QjL3MzN04yNzcDN)章节。
...
:::note
注意：调用这个接口前请先确保已经[登录](/ssl:ttdoc/uYjL24iN/uYzMuYzMuYzM)。
:::
...
```
#### 网页应用
> 如果你之前没有开发过飞书内的网页应用，请先通过[开发文档](/ssl:ttdoc/uYjL24iN/uITO4IjLykDOy4iM5gjM)进行充分了解 。

1. 接口鉴权

**在调用 H5 接口前，请先完成鉴权**。即基于页面 URL，使用固定的算法生成一个字符串——"签名"，并将签名和其他参数传给 **h5sdk.config** 接口，完成鉴权。

> 生成签名及鉴权方法请见[H5JSSDK - 鉴权接口](/ssl:ttdoc/uYjL24iN/uEzM4YjLxMDO24SMzgjN)；可参考文档里的方式，实现一个生成签名的后端接口，用来在 H5 页面上获取签名。
> 
...
``` 
:::note
==具体JSSDK版本号请前往[JSSDK 开发指南](/ssl:ttdoc/uYjL24iN/uITO4IjLykDOy4iM5gjM)获取==
:::
