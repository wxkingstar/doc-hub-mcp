<!--
title: 日志检索
id: 6977562565003214851
fullPath: /tools-and-resources/open-api-log-query
updatedAt: 1690424636000
source: https://open.feishu.cn/document/tools-and-resources/log-query-of-apps/open-api-log-query
-->
# 日志检索
...
3. 在 **服务端日志检索** 页签内，配置查询参数，检索应用调用服务端 API 的日志数据。
    ...
    1. 配置查询参数。
        ...
        - **用户 Open ID**：如果待查询的 API 使用用户访问凭证（user_access_token）进行鉴权，则可以输入相匹配的用户 open_id 进行检索。ID 获取方式参见[用户openID获取指南](/ssl:ttdoc/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/trouble-shooting/how-to-obtain-openid)。
...
3. 在 **事件日志检索** 页签内，配置查询参数，检索应用推送事件的日志数据。
    ...
    1. 配置查询参数。
		...
		- **事件类型**：指定事件的类型。各事件的事件类型，参见[事件列表](/ssl:ttdoc/ukTMukTMukTM/uYDNxYjL2QTM24iN0EjN/event-list)。
...
3. 在 **客户端日志检索** 页签内，配置查询参数，检索应用推送事件的日志数据。
    ...
    1. 配置查询参数。
        ...
        - **用户 Open ID**：应用使用者对应的用户 open_id。ID 获取方式参见[如何获取用户 open_id](/ssl:ttdoc/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/trouble-shooting/how-to-obtain-openid)。
