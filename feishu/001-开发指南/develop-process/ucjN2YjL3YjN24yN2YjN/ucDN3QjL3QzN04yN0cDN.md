<!--
title: 四、设置付费方案
id: 6907569742383628290
fullPath: /uMzNwEjLzcDMx4yM3ATM/ucDN3QjL3QzN04yN0cDN
updatedAt: 1721355316000
source: https://open.feishu.cn/document/isv-guides/publish-your-app/step4-set-billing-plans
-->
# 四、设置付费方案
...
4. 参考[查询订单信息](/ssl:ttdoc/ukTMukTMukTM/uITNwUjLyUDM14iM1ATN) ，将你的应用接入计费系统中。
:::note
  * 对于**增值包**类型的方案，用户购买后，通过[查询用户是否在应用开通范围](/ssl:ttdoc/ukTMukTMukTM/uATNwUjLwUDM14CM1ATN)接口查询时，不会返回**增值包**类型的方案ID，而是返回租户订阅中的其他方案 ID（如免费版或其他付费方案）。
...
## 上架应用中心
应用[完成发布](/ssl:ttdoc/uMzNwEjLzcDMx4yM3ATM/uYjMyUjL2IjM14iNyITN)后，即可使用此付费方案在应用中心销售你的应用。
...
不是，是由 ISV 控制的。针对应用商店应用，飞书提供了[应用商店应用购买](/ssl:ttdoc/uAjLw4CM/ukTMukTMukTM/application-v6/event/public-app-purchase)的事件，在用户购买应用成功后，此事件发送给应用 ISV。ISV 可根据用户的购买时间和购买方案，计算该用户的应用有效使用时长。有效使用时长到期后，用户仍能访问应用，但在产品功能上对用户的限制由 ISV 控制。