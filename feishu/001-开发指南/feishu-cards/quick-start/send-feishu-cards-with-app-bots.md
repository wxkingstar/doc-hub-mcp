<!--
title: 使用指定应用发送飞书卡片
id: 7343499772501360644
fullPath: /uAjLw4CM/ukzMukzMukzM/feishu-cards/quick-start/send-feishu-cards-with-app-bots
updatedAt: 1751876358000
...
:::html
<md-alert type="tip">
如果你仅需向单个指定群组中发送静态卡片，无需用户通过卡片进一步交互，你可选择通过自定义机器人发送卡片，无需企业管理员审核。详情参考[使用自定义机器人发送飞书卡片](/ssl:ttdoc/uAjLw4CM/ukzMukzMukzM/feishu-cards/quick-start/send-message-cards-with-custom-bot)。
</md-alert>
:::
...
1. 进入应用详情页，在左侧导航栏中，进入 **[测试企业与人员](/ssl:ttdoc/home/introduction-to-custom-app-development/testing-enterprise-and-personnel-functions)** 页面，并在页面右上角点击 **创建测试企业** 创建你的测试企业或关联已有测试企业。
:::html
<md-alert type="tip">
你也可以选择[自行创建一个新的企业](https://www.feishu.cn/hc/zh-CN/articles/360043741453-%E5%88%9B%E5%BB%BA%E4%BC%81%E4%B8%9A)，再在新企业中创建应用、添加权限，实现权限免审。
</md-alert>
:::
5. 在 **操作** 列，点击 **关联应用**。
...
:::html
<md-alert type="tip">
如果你使用的是正式版本应用而非测试版本，你需要创建应用版本并发布应用，上述配置才可生效。</md-alert>
:::
### 步骤二：创建卡片
...
你可参考[卡片 JSON 2.0 结构](/ssl:ttdoc/uAjLw4CM/ukzMukzMukzM/feishu-cards/card-json-v2-structure)、[卡片 JSON 2.0 版本组件概述](/ssl:ttdoc/uAjLw4CM/ukzMukzMukzM/feishu-cards/card-json-v2-components/component-json-v2-overview)等文档构建卡片 JSON。以下是卡片 JSON 示例：
...
:::html
<md-alert type="tip">
- 在步骤一中，如果你使用的是应用的测试版本而非正式版本，你需要测试版本关联的测试人员账号调用开放平台接口。
...
1. 在[发送消息 API](/ssl:ttdoc/uAjLw4CM/ukTMukTMukTM/reference/im-v1/message/create) 中，配置请求参数。
    ...
    - 对于搭建工具搭建的卡片，参考以下请求体示例和表格，将示例值修改为实际值：
        ...
        <md-table> 
          ...
          <md-tbody> 
        	...
        	receive_id
              ...
              <md-td>是</md-td>  
              <md-td>
                消息接收者的 ID。你可 **查询参数** 页签下的 **receive_id_type** 处，点击 **快速复制 open_id**，在弹窗中，选择要发送消息的测试成员。
        <BR>**注意**：消息接收者必须在应用的可用范围内。在本教程中：
       <BR>- 如果你使用的是应用的正式版本，请复制应用创建者的 open_id；
       <br>- 如果你使用的是应用测试版本，请使用测试人员账号登录 API 调试台后，复制测试人员的 open_id。详情可参见[配置应用可用范围](/ssl:ttdoc/home/introduction-to-scope-and-authorization/availability)。<br>
           ![](//sf3-cn.feishucdn.com/obj/open-platform-opendoc/daa1f596aad20ca0091e19c16dd72e2f_kX9jyznlZj.png?height=1152&lazyload=true&maxWidth=344&width=1748)
            </md-td> 
            </md-tr>  
            <md-tr> 
            <md-td>
        ...
        消息类型。在卡片场景下，消息类型为 `interactive`。
        	...
        	content
              ...
              <md-td>是</md-td>  
              <md-td>
                消息内容。此处传入卡片相关内容。要发送由搭建工具搭建的卡片，参考以下说明。
            </md-td> 
            </md-tr>  
            <md-tr>
            <md-td>
           ...
           <md-td>否</md-td>
       <md-td>
        卡片类型，发送由搭建工具搭建的卡片（也称卡片模板），固定取值为 `template`。
       </md-td>
         </md-tr>
                <md-tr>
       <md-td>
       ...
       <md-td>
        ...
        <md-td>是</md-td>
        <md-td>
          搭建工具中创建的卡片（也称卡片模板）的 ID，如 AAqigYkzabcef。可在搭建工具中通过复制卡片 ID 获取。   	<br>![](//sf3-cn.feishucdn.com/obj/open-platform-opendoc/0095016591a8fce17627f6472a0c6dc0_gUrQmjS8mh.png?height=322&lazyload=true&maxWidth=500&width=1725)
            ...
            <md-td>否</md-td>  
            <md-td>
				搭建平台中创建的卡片的版本号，如 `1.0.0`。<br> ![](//sf3-cn.feishucdn.com/obj/open-platform-opendoc/3b670e4057d410831b7afbb6fb83029d_C3hlpbiDeq.png?height=308&lazyload=true&maxWidth=500&width=1713)
          <br>**注意**：
            ...
            <md-td>否</md-td>  
            <md-td>
				若卡片绑定了变量，你需在该字段中传入实际变量数据的值，对应搭建工具中 **模拟数据** 的值。详情参考[配置卡片变量](/ssl:ttdoc/uAjLw4CM/ukzMukzMukzM/feishu-cards/feishu-card-cardkit/configure-card-variables)。
            </md-td> 
            </md-tr> 
...
- 如果业务场景需要卡片基于用户操作即时更新，推荐参考[开发一个卡片交互机器人](/ssl:ttdoc/uAjLw4CM/uMzNwEjLzcDMx4yM3ATM/develop-a-card-interactive-bot/introduction)教程与[示例代码](/ssl:ttdoc/uAjLw4CM/uMzNwEjLzcDMx4yM3ATM/develop-a-card-interactive-bot/explanation-of-example-code)。
- 除发送消息接口外，你还可以通过应用调用其它接口发送卡片。支持发送卡片的接口如下表所示：
  ...
  <md-table>
    ...
    <md-tbody>
      <md-tr>
        ...
        <md-td>服务端 API</md-td>
        <md-td>支持向机器人所在的群聊、拥有机器人可用性的用户单聊内发送消息卡片。</md-td>
      </md-tr>
        <md-tr>
        <md-td>
          [批量发送消息](/ssl:ttdoc/ukTMukTMukTM/ucDO1EjL3gTNx4yN4UTM)
        </md-td>
        <md-td>服务端 API</md-td>
        <md-td>支持向多个用户或者多个部门中的成员发送卡片。</md-td>
      </md-tr>
      <md-tr>
        <md-td>
        ...
        <md-td>服务端 API</md-td>
        <md-td>用于机器人在群会话中发送仅指定用户可见的消息卡片。卡片上将展示 **仅对你可见** 标识。</md-td>
      </md-tr>
      <md-tr>
        <md-td>
        ...
        <md-td>服务端 API</md-td>
        <md-td>配合发送临时卡片使用，用于销毁不需要的临时消息。</md-td>
      </md-tr>
      <md-tr>
        <md-td>
  ...
  **注意**：不要与 **延时更新消息卡片** 接口混用。
      ...
      <md-tr>
        <md-td>
          [sendMessageCard](/ssl:ttdoc/uYjL24iN/uUjN5UjL1YTO14SN2kTN)
        </md-td>
        <md-td>客户端 API，必须在小程序或网页应用中调用</md-td>
        <md-td>
