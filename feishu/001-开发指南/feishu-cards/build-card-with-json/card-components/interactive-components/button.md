<!--
title: 按钮
id: 7343499772501180420
fullPath: /uAjLw4CM/ukzMukzMukzM/feishu-cards/card-components/interactive-components/button
updatedAt: 1735814891000
...
本文档介绍按钮组件的 JSON 1.0 结构，要查看新版 JSON 2.0 结构，参考[按钮](/ssl:ttdoc/uAjLw4CM/ukzMukzMukzM/feishu-cards/card-json-v2-components/interactive-components/button)。
...
- 在[卡片 JSON 1.0 结构](/ssl:ttdoc/uAjLw4CM/ukzMukzMukzM/feishu-cards/card-json-structure)中，若按钮组件直接位于卡片根节点，而非嵌套在其它组件中，你需将其 JSON 数据配置在[交互模块](/ssl:ttdoc/ukTMukTMukTM/uYzM3QjL2MzN04iNzcDN/component-list/common-components-and-elements)（`"tag": "action"`）的 `actions` 字段中使用。

- [卡片 JSON 2.0 结构](/ssl:ttdoc/uAjLw4CM/ukzMukzMukzM/feishu-cards/card-json-v2-structure)已不支持[交互模块](/ssl:ttdoc/ukTMukTMukTM/uYzM3QjL2MzN04iNzcDN/component-list/common-components-and-elements)（`"tag": "action"`）相关属性。你可直接将按钮放置于 `elements` 中，并配置合适的[组件间距 (vertical_spacing 和 horizontal_spacing)](/ssl:ttdoc/uAjLw4CM/ukzMukzMukzM/feishu-cards/card-json-v2-breaking-changes-release-notes#a967672) 使用。
## 嵌套规则
...
<md-tr>
      ...
      <md-td>Struct</md-td>
      <md-td>/</md-td>
      <md-td>配置交互类型和具体交互行为。支持同时生效跳转链接和回传交互。详情参考[配置卡片交互](/ssl:ttdoc/uAjLw4CM/ukzMukzMukzM/feishu-cards/configuring-card-interactions)。</md-td>
    </md-tr>
</tbody>
</table>
:::
...
<md-td>Struct</md-td>
  <md-td>/</md-td>
<md-td>
基于 url 元素配置多端跳转链接，详情参见旧版文档[url 元素](/ssl:ttdoc/ukTMukTMukTM/uYzM3QjL2MzN04iNzcDN/component-list/common-components-and-elements#09a320b3)。该字段与 `url` 字段不可同时设置。
</md-td>
</md-tr>
<md-tr>
<md-td>value</md-td>
...
为按钮组件成功[配置卡片交互](/ssl:ttdoc/uAjLw4CM/ukzMukzMukzM/feishu-cards/configuring-card-interactions)后，用户基于按钮组件进行交互时，你在开发者后台配置的请求地址将会收到回调数据。
