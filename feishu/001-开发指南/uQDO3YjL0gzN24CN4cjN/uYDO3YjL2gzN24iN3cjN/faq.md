<!--
title: 常见问题
id: 7509692063405146140
fullPath: /uYjL24iN/uYDO3YjL2gzN24iN3cjN/faq
updatedAt: 1748507323000
...
A：应用需要申请对应权限 —— 查看、评论、编辑和管理云文档所有文件，参考[开始使用](/ssl:ttdoc/uYjL24iN/uYDO3YjL2gzN24iN3cjN/introduction)权限部分。
...
- [方法二：通过接口以应用身份创建文档](/ssl:ttdoc/ukTMukTMukTM/ugDM2YjL4AjN24COwYjN)
...
<md-tbody>
<md-tr>
<md-td>页面访问第一篇文档（预计优化200~400ms）</md-td>
<md-td>
1.  用户第一次授权后将 refresh_token 存在服务端；
1. 用户第二次访问时，接入方可在「模板服务」中提前通过用户 refresh_token 进行用户身份授权，将生成的签名信息放在模板中；
1. 模板到达客户端时，利用模板中签名信息调用 WebComponent.config 接口进行鉴权；
</md-td>
<md-td>
1.  接入方可在「模板服务」中提前进行应用身份授权，将生成的签名信息放在模板中；
1. 模板到达客户端时，利用模板中签名信息调用 WebComponent.config 接口进行鉴权；
</md-td>
</md-tr>
  <md-tr>
<md-td>页面访问第二篇文档（预计优化3~5s）</md-td>
<md-td>
- 调用 [切换文档](/ssl:ttdoc/uYjL24iN/uYDO3YjL2gzN24iN3cjN/replace-url)快速访问新文档
</md-td>
<md-td>
</md-td>
</md-tr>
...
1. 通过 openAPI 获取文档的正确域名 「[获取文档元数据 - 服务端文档 - 开发文档 - 飞书开放平台](/ssl:ttdoc/uAjLw4CM/ukTMukTMukTM/reference/drive-v1/meta/batch_query)」
...
1. 先通过配置隐藏目录（[功能配置](/ssl:ttdoc/uYjL24iN/uYDO3YjL2gzN24iN3cjN/feature-config)）
1. 通过事件监听获取目录数据（[文档目录变化](/ssl:ttdoc/uYjL24iN/uYDO3YjL2gzN24iN3cjN/event-listener/document-directory-change)）
1. 通过调用跳转锚点接口（[锚点跳转](/ssl:ttdoc/uYjL24iN/uYDO3YjL2gzN24iN3cjN/invoke-api/anchor-jump)）
...
A: 当前云文档组件私有化部署只支持web场景，不支持小程序场景。私有化部署接入时需要配置一下当前环境的私有化域名，[相关文档](/ssl:ttdoc%2FuYjL24iN%2FuYDO3YjL2gzN24iN3cjN%2Fquick-start-ka)
...
目前不支持直接加载wiki文档，对wiki文档，可以通过wiki接口拿到对应文档类型的token，使用对应文档的token 和类型 组 url 来使用云文档组件 wiki接口：[获取知识空间节点信息](/ssl:ttdoc/ukTMukTMukTM/uUDN04SN0QjL1QDN/wiki-v2/space/get_node)
