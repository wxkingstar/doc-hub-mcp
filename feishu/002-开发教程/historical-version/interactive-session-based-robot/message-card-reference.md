...
id: 6999971130074529820
fullPath: /home/interactive-session-based-robot/message-card-reference
updatedAt: 1687936492000
...
2. 识别数据中的事件内容。
    ...
    由于订阅的事件可能不止一个，因此需要通过 **接收消息** 事件的特征，识别该事件。根据 [接收消息](/ssl:ttdoc/uAjLw4CM/ukTMukTMukTM/reference/im-v1/message/events/receive) 事件文档，获取该事件的关键特征：事件类型（`header.event_type`）为`im.message.receive_v1`。
...
5. 调用开发平台的[回复消息](/ssl:ttdoc/uAjLw4CM/ukTMukTMukTM/reference/im-v1/message/reply)接口，将保存的百科信息通过`message_id`回复给用户。
